...
def data_ch(yt_ch_id):
    ...
    ch_request = youtube.channels().list(part="snippet,contentDetails,statistics,topicDetails",
                                         id=yt_ch_id)
    ...
    request = youtube.playlistItems().list(part="snippet", playlistId=(ch_info["Channel Name"]["Playlist_Id"]),
                                           maxResults=50)
    ...
    for i in range(len(vid_id)):
        ...
        vi_tags = (vid_response['items'][0]['snippet']['tags']) if (vid_response['items'][0]['snippet']) == [
            'tags'] else "Not Available"
        vi_pub = (vid_response['items'][0]['snippet']['publishedAt'])
        vi_view = (vid_response['items'][0]['statistics']['viewCount'])
        vi_like = (vid_response['items'][0]['statistics']['likeCount'])
...
def sql_conv(result):
    ...
    pl_list=[]
    pl_sql = {"Playlist_Id": result['Channel Name']['Playlist_Id'],
                         "Channel_Id": result['Channel Name']['Channel_Id']}
...
def table(df):
    ...
    engine = create_engine('mysql+mysqlconnector://root:12345@localhost/youtube', echo=False)
